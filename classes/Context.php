...
class ContextCore
{
    ...
    public function updateCustomer(Customer $customer)
    {
        ...
        if (Configuration::get('PS_CART_FOLLOWING') && (empty($this->cookie->id_cart) || Cart::getNbProducts($this->cookie->id_cart) == 0) && $idCart = (int) Cart::lastNoneOrderedCart($this->customer->id)) {
            ...
            foreach($this->cart->getCartRules(CartRule::FILTER_ACTION_ALL, false) as $row){
                if (Validate::isLoadedObject($row['obj']) && (!$row['obj']->checkValidity($this->getContext(), false, false, true))) {
                    $this->cart->removeCartRule($row['id_cart_rule']);
                }
            }
        } else {
